...
<div class="wpfl-3 wpforo-section">
	...
	foreach( $forums as $key => $forum ) :
		...
		$topics     = WPF()->topic->get_topics(
			apply_filters(
				'wpforo_tpl_l3_forum_loop_intro_topics_get_args',
        ...
        <div id="wpf-forum-<?php echo $forum['forumid'] ?>" class="forum-wrap <?php wpforo_unread( $forum['forumid'], 'forum' ) ?>">
            <div class="wpforo-forum">
                ...
                <div class="wpforo-forum-stat">
                    ...
                    <?php if( $has_topics ) : ?>
                    <div class="wpft-row">
                        <div class="wpft-cell-left wpfcl-6" style="margin-top:1px;">&nbsp;</div>
                        <div class="wpft-cell-right" style="color: rgb(67, 166, 223);font-size: 16px; cursor: pointer; line-height: 20px;">
                            <i id="img-arrow-<?php echo intval( $forum['forumid'] ) ?>" class="topictoggle fas fa-chevron-<?php echo( $topic_toglle == 1 ? 'up' : 'down' ); ?>"></i>
                        </div>
                    </div>
                    <?php endif ?>
                </div>
                        ...
                        <ul>
							<?php foreach( $topics as $topic ) : ?>
								...
								<?php if( $topic_url && ! empty( $member ) ): ?>
                                    <li class="<?php wpforo_topic_types( $topic );
									wpforo_unread( $topic['topicid'], 'topic' ) ?>">
                                        <div class="wpforo-last-topic wpfcl-2">
                                            <div class="wpf-tbox votes <?php if( $topic['answers'] > 0 ) echo "wpfcl-4" ?>" wpf-tooltip="<?php wpforo_phrase( 'Votes' ) ?>" wpf-tooltip-position="top"
                                                 wpf-tooltip-size="small">
                                                <div class="wpforo-label">
                                                    <svg class="<?php if( $topic['answers'] > 0 ) echo "wpfcl-4" ?>" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                            ...
                                            <div class="wpf-tbox answers <?php if( $topic['answers'] > 0 ) echo "wpfcl-5" ?>" wpf-tooltip="<?php wpforo_phrase( 'Answers' ) ?>"
                                                 wpf-tooltip-position="top" wpf-tooltip-size="small">
                                                <div class="wpforo-label">
                                                    <svg class="<?php if( $topic['answers'] > 0 ) echo "wpfcl-5" ?>" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <div class="wpforo-last-topic-title">
                                            ...
                                            <div class="wpforo-topic-author">
                                                ...
                                                <span class="wpforo-last-topic-info wpfcl-2">
													<?php wpforo_member_link( $member, 'by' ); ?><span class="wpforo-date wpforo-date-ago">, <?php wpforo_date( $topic['modified'] ); ?></span>
                                                </span>
                                            </div>
                                        ...
                                        <div class="wpforo-last-topic-status wpfcl-2 <?php if( $topic['type'] ) echo "wpfbr-d-10" ?> <?php if( $topic['solved'] && ! $topic['type'] ) echo "wpfbr-d-8" ?>">
											<?php echo str_replace( 'fa-check-circle', 'fas fa-square-check', wpforo_topic_icon( $topic, 'all', true, false ) ) ?>
                                        </div>
                                    </li>
									<?php do_action( 'wpforo_tpl_l3_forum_loop_intro_topics_after', $topic, $topic_url, $member, $forum ); ?>
								<?php endif; ?>
							<?php endforeach; ?>
