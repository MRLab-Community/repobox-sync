<?php
// ✅ DEBUG MODE: Log su file, NON a schermo
error_reporting(E_ALL);
ini_set('display_errors', 0);
if (session_status() === PHP_SESSION_NONE) session_start();

// Includi config se esiste
...
$userRole = isset($_SESSION['repobox_role']) ? strtolower($_SESSION['repobox_role']) : '';
$isAdmin = ($loggedIn && ($userRole === 'admin' || $userRole === 'administrator'));

$baseDir = defined('REPOBOX_PROJECTS_DIR') ? REPOBOX_PROJECTS_DIR : __DIR__ . '/progetti';

// --- LETTURA COLORI DAL DATABASE ---
$headerColor = '#3f8e9b'; // Default
$sidebarColor = '#ffffff'; // Default

if (file_exists(__DIR__ . '/installed.json') && file_exists(__DIR__ . '/config_db.php')) {
    try {
        // Ricreiamo la connessione PDO qui se necessario, poiché getDB() potrebbe essere scope-local in login.php
        if (!isset($pdo)) {
            $dsn = "mysql:host=" . REPO_DB_HOST . ";dbname=" . REPO_DB_NAME . ";charset=utf8mb4";
            $pdo = new PDO($dsn, REPO_DB_USER, REPO_DB_PASS, [
        ...
        if (!empty($settings['header_color'])) {
            $headerColor = htmlspecialchars($settings['header_color']);
        }
        if (!empty($settings['sidebar_color'])) {
            $sidebarColor = htmlspecialchars($settings['sidebar_color']);
        }
        
        debugLog("Colori caricati dal DB", ['header' => $headerColor, 'sidebar' => $sidebarColor]);

    } catch (Exception $e) {
        debugLog("Errore lettura colori: " . $e->getMessage());
...
.sidebar { 
    width: 100%; 
    background: <?php echo $sidebarColor; ?>; /* <-- Deve usare la variabile */
...
.site-header { 
    background-color: <?php echo $headerColor; ?>; /* <-- Deve usare la variabile */
    color: white; 
    text-align: left; 
    padding: 0.8em 0; 
    width: 100%; 
...
function refreshTree() {
    ...
    document.querySelectorAll('#fileTree li').forEach(li => {
        ...
        // Se c'è una sottolista ed è visibile (display block), salva il percorso
    ...
    fetch('?action=list').then(r => r.json()).then(data => {
        ...
        ul.innerHTML = '';
        // 3. RICOSTRUISCI PASSANDO LA LISTA DELLE CARTELLE APERTE
...
function buildTree(data, parent, currentPath = '', openFolders = []) {
    for (const name in data) {
        ...
        // Salva il percorso nell'elemento DOM per poterlo leggere dopo
        ...
        if (item.type === 'dir') {
            ...
            // Click sulla cartella per aprire/chiudere
            span.onclick = (e) => {
                e.stopPropagation();
            ...
            // Pulsanti azione (solo se loggato)
            <?php if ($loggedIn): ?>
